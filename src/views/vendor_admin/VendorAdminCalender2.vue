...
const calendarOptions = ref({
  ...
  eventClick: (info) => {
    ...
    editEventEndDate.value = info.event.end ? moment(info.event.end).format("YYYY-MM-DD") : editEventStartDate.value;
    editEventEndTime.value = info.event.end ? moment(info.event.end).format("HH:mm") : "13:00";
  ...
  eventDrop: async (info) => {
    try {
      const updatedEvent = {
        ...
        start_time: moment(info.event.start).utc().toISOString(),
        end_time: info.event.end ? moment(info.event.end).utc().toISOString() : moment(info.event.start).utc().toISOString(),
